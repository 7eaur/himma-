...
import DevelopmentAudioSkipAction from "@/components/DevelopmentAudioSkipAction";
...
function completionCopy(levelId: number) {
  if (levelId === 1) {
    return {
      title: "أحسنت، أكملت الاستعداد للقراءة",
      text: "خطوتك التالية هي بناء الكلمة. استمر بنفس التركيز.",
      cta: "الانتقال إلى خطوتي التالية",
    };
  }
  if (levelId === 2) {
    return {
      title: "أحسنت، أكملت بناء الكلمة",
      text: "خطوتك التالية هي الطلاقة والفهم. استمر، أنت تتقدم بشكل ممتاز.",
      cta: "الانتقال إلى خطوتي التالية",
    };
  }
  return {
    title: "أحسنت، أكملت المستوى الثالث",
    text: "أكملت أنشطة الطلاقة والفهم. أصبح مسارك جاهزًا للاختبار البعدي عندما يفتحه المشرف.",
    cta: "العودة إلى مساري",
  };
}
...
export default function StudentActivityPage() {
  ...
  const operationKey = (kind: "answer" | "upload") =>
    `himma:activity:${sessionId}:${step?.id ?? 0}:${(view?.attempts_used ?? 0) + 1}:${kind}`;
  ...
  const toggleOption = (id: number) => {
    ...
    if (ORDER.has(interaction)) {
      setSelected((current) =>
        current.includes(id) || (targetCount > 0 && current.length >= targetCount) ? current : [...current, id],
      );
    }
  };
  ...
  const submitStructured = async (declaredMediaGapSkip = false) => {
    if (!view || !step || !interaction) return;
    setSubmitting(true);
    setError("");
    ...
    try {
      const response = await fetch(`/api/activities/session/${sessionId}/attempt/${view.item_id}/submit`, {
        ...
        body: JSON.stringify({
          ...
          hint_used: view.retry,
          elapsed_seconds: elapsed,
          declared_media_gap_skip: declaredMediaGapSkip,
        }),
      });
  ...
  if (done) {
    const levelId = progress?.level_id ?? 1;
    const copy = completionCopy(levelId);
    return (
      <div className={styles.resultPage} dir="rtl" data-testid="activity-session" data-phase="done">
        <div className={styles.resultCard}>
          <div className={styles.resultContent}>
            <h1 className={styles.resultTitle}>{copy.title}</h1>
            <p className={styles.resultText}>{copy.text}</p>
            <button className={styles.primary} onClick={() => router.push("/student")}>{copy.cta}</button>
          </div>
          <div className={styles.resultVisual}>
            <Image src="/characters/girl/success.png" alt="شخصية هِمّة تحتفل" width={320} height={390} />
          </div>
        </div>
      </div>
    );
    ...
    return (
      <div className={styles.page} dir="rtl" data-testid="activity-session" data-phase="error">
        <div className={styles.loadingState}>
          ...
          <p>{error || "بيانات النشاط غير متاحة."}</p>
          <button className={styles.primary} onClick={() => void loadCurrent()}>حاول مرة أخرى</button>
        </div>
      </div>
    );
  }
  ...
  const isImageChoice = interaction === "choose_image"
    || interaction === "listen_choose_image"
    || ((interaction === "choose_many" || interaction === "listen_choose_many") && imageOptions.length > 0);
  ...
  return (
    <div
      ...
      data-phase={submitting ? "submitting" : "active"}
      data-activity-kind={isReinforcement ? "reinforcement" : "core"}
    >
      <header className={styles.header}>
        <div className={styles.headerInner}>
          <Image src="/brand/logo-navy.svg" alt="هِمّة" width={124} height={44} priority />
          <button className={styles.exit} type="button" onClick={() => router.push("/student")}>
            <LogOut size={21} /><span>رجوع</span>
          </button>
        </div>
      </header>
      ...
      <div className={styles.progressPanel}>
        ...
        <div className={styles.progressTrack} aria-label={`التقدم ${percent}%`}>
          <div className={styles.progressFill} style={{ width: `${Math.max(percent, 2)}%` }} />
        </div>
      </div>
      ...
      <main className={styles.shell}>
        <section className={styles.card}>
          ...
          <div className={styles.contentColumn}>
            {isReinforcement && (
              <div className={styles.notice} data-testid="reinforcement-intro">
                هذا تدريب قصير يساعدك على إتقان المهارة، وبعد إتقانها تعود إلى نشاطك الأساسي.
              </div>
            )}

            <h1 className={styles.questionTitle}>{view.round.question_text}</h1>

            {stimulus && !LISTEN.has(interaction) && !READ.has(interaction) && (
              <div className={`${styles.stimulusBox} ${stimulus.length <= 3 ? styles.letterStimulus : ""}`}>{stimulus}</div>
            )}

            {visualAsset && interaction !== "memory_sequence" && (
              <div className={styles.contextImage}>
                <Image src={visualAsset.url} alt={visualAsset.semantic_text || "صورة النشاط"} width={420} height={260} unoptimized />
              </div>
            )}
            ...
            {LISTEN.has(interaction) && (
              <button
                type="button"
                className={`${styles.listenButton} ${isListening ? styles.listenPulse : ""}`}
                onClick={() => void playPrompt()}
                disabled={isListening || !audioAssets.length}
                data-testid="activity-listen-prompt"
              >
            ...
            {READ.has(interaction) && (
              <div
                className={`${styles.readingBox} ${(step.expected_reading_text?.length || stimulus.length) > 55 ? styles.readingBoxLong : ""}`}
                data-testid="activity-reading-text"
              >
                {step.expected_reading_text || stimulus || "اقرأ النص الظاهر"}
              </div>
            )}

            <div className={styles.instructionRow} data-testid="student-task-instruction">
              <Info size={21} aria-hidden="true" />
              <p>{view.round.instruction_text}</p>
            </div>

            {hasMediaGap && (
              <div className={styles.notice} data-testid="declared-media-gap">
                هذا الملف غير متوفر ضمن الوسائط المعتمدة حاليًا.
                <div className={styles.inlineActions}>
                  <button className={styles.secondary} type="button" onClick={() => void submitStructured(true)} disabled={submitting}>
                    متابعة دون احتساب الجولة
                  </button>
                </div>
              </div>
            )}
            ...
            {!hasMediaGap && interaction === "memory_sequence" && (
              memoryPreview ? (
                <>
                  <div className={styles.imageOptions} data-testid="activity-memory-preview">
                    {imageOptions.map((asset, index) => (
                      <div key={asset.asset_id} className={styles.imageOption}>
                        <span className={styles.selectedMark}>{index + 1}</span>
                        <Image src={asset.url} alt={asset.semantic_text || `الصورة ${index + 1}`} width={220} height={150} unoptimized />
                        <span className={styles.imageLabel}>{asset.semantic_text}</span>
                      </div>
                    ))}
                  </div>
                  <div className={styles.inlineActions}>
                    <button className={styles.primary} type="button" onClick={() => setMemoryPreview(false)}>التالي</button>
                  </div>
                </>
              ) : (
                <>
                  <div className={styles.sequenceBoard}>
                    {selected.length === 0
                      ? <span className={styles.sequenceHint}>رتّب الصور كما ظهرت.</span>
                      : selected.map((id, index) => {
                        const asset = imageOptions.find((candidate) => Number(candidate.option_id) === id);
                        return <span className={styles.sequenceChip} key={id}><span className={styles.number}>{index + 1}</span>{asset?.semantic_text}</span>;
                      })}
                  </div>
                  <div className={styles.imageOptions} data-testid="activity-sequence-image-options">
                    {imageOptions
                      .filter((asset) => !selected.includes(Number(asset.option_id)))
                      .map((asset) => (
                        <button
                          key={asset.asset_id}
                          className={styles.imageOption}
                          type="button"
                          onClick={() => toggleOption(Number(asset.option_id))}
                          disabled={selectionLimitReached}
                        >
                          <Image src={asset.url} alt={asset.semantic_text || "خيار صورة"} width={220} height={150} unoptimized />
                          <span className={styles.imageLabel}>{asset.semantic_text}</span>
                        </button>
                      ))}
                  </div>
            ...
            {!hasMediaGap && interaction !== "memory_sequence" && (interaction === "sequence" || interaction === "build_word") && (
              <>
                <div className={styles.sequenceBoard}>
                  {selected.length === 0
                    ? <span className={styles.sequenceHint}>ابدأ بالعنصر الأول ثم أكمل بالترتيب.</span>
                    : selected.map((id, index) => (
                      <span className={styles.sequenceChip} key={id}>
                        <span className={styles.number}>{index + 1}</span>
                        {options.find((option) => option.id === id)?.text}
                      </span>
                    ))}
                </div>
                {sequenceImagesComplete && interaction !== "build_word" ? (
                  <div className={styles.imageOptions} data-testid="activity-sequence-image-options">
                    {imageOptions
                      .filter((asset) => !selected.includes(Number(asset.option_id)))
                      .map((asset) => (
                  ...
                  <div className={styles.options} data-testid="activity-sequence-options">
                    {options
            ...
            {!hasMediaGap && interaction !== "memory_sequence" && !ORDER.has(interaction) && !READ.has(interaction) && isImageChoice && (
              <div className={styles.imageOptions} data-testid="activity-image-options">
                {imageOptions.map((asset) => {
                  ...
                  const capped = MULTI.has(interaction) && selectionLimitReached && !chosen;
                  return (
                    <button
                      key={asset.asset_id}
                      className={`${styles.imageOption} ${chosen ? styles.optionSelected : ""}`}
                      type="button"
                    ...
                    >
                      {chosen && <span className={styles.selectedMark}><Check size={18} /></span>}
                      <Image src={asset.url} alt={asset.semantic_text || "خيار مصور"} width={220} height={150} unoptimized />
                      <span className={styles.imageLabel}>{asset.semantic_text || options.find((option) => option.id === id)?.text}</span>
                    </button>
                  );
                })}
              </div>
            ...
            {!hasMediaGap && interaction !== "memory_sequence" && !ORDER.has(interaction) && !READ.has(interaction) && !isImageChoice && (
              <div className={styles.options} data-testid="activity-text-options">
                {options.map((option) => {
                  ...
                  return (
                    <button
                      key={option.id}
                      className={`${styles.option} ${chosen ? styles.optionSelected : ""}`}
                      type="button"
                      onClick={() => toggleOption(option.id)}
                      disabled={capped}
                      aria-pressed={chosen}
              ...
              <div className={styles.recordPanel}>
                {!audioBlob ? (
                  <>
                    ...
                    {isRecording && (
                      <p className={styles.timer}>{String(Math.floor(recordingSeconds / 60)).padStart(2, "0")}:{String(recordingSeconds % 60).padStart(2, "0")}</p>
                    )}
                  </>
                ) : (
                  <>
                    ...
                    <div className={styles.inlineActions}>
                      <button className={styles.secondary} type="button" onClick={resetRoundState}>
                        <RotateCcw size={17} /> إعادة التسجيل
                      </button>
                      <button className={styles.primary} type="button" onClick={() => void uploadReading()} disabled={submitting}>
                        {submitting ? "جاري الحفظ..." : "إرسال التسجيل"}
                      </button>
                    </div>
                  </>
                )}

                <DevelopmentAudioSkipAction
                  sessionId={sessionId}
                  itemId={view.item_id}
                  stepId={step.id}
          ...
          <aside className={styles.coach} aria-label="نصيحة هِمّة">
            <div className={styles.tip}><span>{message}</span></div>
            <Image
              className={styles.character}
              src={view.retry ? "/characters/girl/encourage.png" : "/characters/girl/explain.png"}
              alt="شخصية هِمّة"
              width={150}
              height={205}
            />
          ...
          {!hasMediaGap && !READ.has(interaction) && !(interaction === "memory_sequence" && memoryPreview) && (
            <div className={styles.bottomActions}>
              {ORDER.has(interaction) && selected.length > 0 && (
                <button className={styles.secondary} type="button" onClick={() => setSelected([])} disabled={submitting}>
                  <RotateCcw size={17} /> إعادة الترتيب
                </button>
              )}
              <button className={styles.primaryWide} type="button" onClick={() => void submitStructured(false)} disabled={submitting || !canSubmit}>
                {submitting ? "جاري الحفظ..." : "تأكيد والمتابعة"}
              </button>
            </div>
          )}
        </section>
