...
export default function StudentActivityPage() {
  ...
  const loadCurrent = useCallback(async () => {
    ...
    try {
      ...
      setView(data); resetRoundState(); await fetchProgress();
  ...
  const submitStructured = async (declaredMediaGapSkip = false) => {
    ...
    try {
      const response = await fetch(`/api/activities/session/${sessionId}/attempt/${view.item_id}/submit`, { method: "POST", headers: { "Content-Type": "application/json", "Idempotency-Key": idempotencyKey("answer") }, body: JSON.stringify({ step_id: step.id, selected_option_ids: selected, hint_used: view.retry, elapsed_seconds: elapsed, declared_media_gap_skip: declaredMediaGapSkip }) });
  ...
  if (done) { const levelId = progress?.level_id ?? 1; const copy = completionCopy(levelId); return <div className={styles.resultPage} dir="rtl"><div className={styles.resultCard}><div className={styles.resultContent}><h1 className={styles.resultTitle}>{copy.title}</h1><p className={styles.resultText}>{copy.text}</p><button className={styles.primary} onClick={() => router.push("/student")}>{copy.cta}</button></div><div className={styles.resultVisual}><Image src="/characters/girl/success.png" alt="شخصية هِمّة تحتفل" width={320} height={390} /></div></div></div>; }
  if (!view || !step || !interaction) return <div className={styles.page} dir="rtl"><div className={styles.loadingState}><h1>تعذر فتح النشاط</h1><p>{error || "بيانات النشاط غير متاحة."}</p><button className={styles.primary} onClick={() => void loadCurrent()}>حاول مرة أخرى</button></div></div>;
  ...
  return <div className={styles.page} dir="rtl" data-testid="activity-session" data-phase="active" data-activity-kind={isReinforcement ? "reinforcement" : "core"}>
    ...
    <main className={styles.shell}><section className={styles.card}>
      ...
      <div className={styles.contentColumn}>
        ...
        {READ.has(interaction) && <div className={`${styles.readingBox} ${(step.expected_reading_text?.length || stimulus.length) > 55 ? styles.readingBoxLong : ""}`}>{step.expected_reading_text || stimulus || "اقرأ النص الظاهر"}</div>}
