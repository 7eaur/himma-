...
export default function AudioReviewPage() {
  ...
  return <AdminPage>
    ...
    {loading ? <AdminPanel><p>جاري تحميل التسجيلات...</p></AdminPanel> : submissions.length === 0 ? <AdminEmptyState title="لا توجد تسجيلات تنتظر المراجعة" description={studentFilter ? "لا توجد حاليًا تسجيلات معلقة لهذا الطالب." : "ستظهر هنا التسجيلات الحالية فقط عندما تحتاج إلى قرار المشرف."} action={<Headphones size={24} aria-hidden="true" />} /> : <div className="space-y-4">{submissions.map((submission) => <AdminPanel key={submission.id}>
